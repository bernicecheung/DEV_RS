...
This script apply space smoothing and high pass filting to fMRIPrep output
"""
...
parser.add_argument(
    '--base_dir', 
    action='store', 
    default='/projects/sanlab/shared/DEV/bids_data',
...
parser.add_argument(
    ...
    action='store',
    type=float,
    default=64.103, # for dev_rs tr = 0.78
    help='Sigma in volumn (filter cut off in 1 / 2*TR *cutoff_in_hz).')
parser.add_argument(
    '--n_procs',
    action='store',
    default=1,
...
output_dir = os.path.join(base_dir,'rs_postfmriprep') # the output directory, I made it to be parallel with the derivative folder 
...
os.makedirs(os.path.join(output_dir,'working',subj_id, wave_id, acq_id), exist_ok = True)
wf = pe.Workflow(name = 'datainput')
wf.base_dir = os.path.join(output_dir,'working',subj_id, subj_id, wave_id, acq_id)

# Feed the dynamic parameters into the node
inputnode = pe.Node(
    interface = niu.IdentityInterface(
    fields = ['subj_id','task_id','wave_id','acq_id','bold_space','brainmask','fwhm']), name = 'inputnode')
# specified above
inputnode.inputs.subj_id = subj_id
inputnode.inputs.wave_id = wave_id
inputnode.inputs.acq_id = acq_id
...
datasource.inputs.template_args = dict(
    func=[['subj_id', 'wave_id','subj_id', 'wave_id','task_id', 'acq_id', 'bold_space']],
    brainmask =[['subj_id', 'wave_id','subj_id', 'wave_id','task_id', 'acq_id', 'bold_space']])
datasource.inputs.field_template = dict(
    func = 'fmriprep/sub-%s/ses-%s/func/sub-%s_ses-%s_task-%s_acq-%s_bold_space-%s_preproc.nii.gz',
    brainmask = 'fmriprep/sub-%s/ses-%s/func/sub-%s_ses-%s_task-%s_acq-%s_bold_space-%s_preproc.nii.gz')
